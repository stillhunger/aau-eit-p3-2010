...
Det andet trin skal forstærke et signal med en maksimal peakspænding på 316 mV op til 2 V, altså 6,33 gange. Belastningsmodstanden for dette forstærkertrin er bestemt af indgangsvælgeren, som er det næste trin efter forforstærkeren. Indgangsvælgerens indgangsimpedans er 30,8 k\ohm \fixme{reference til indgangsvælgeren hvor det står}. $R_S$ er i dette trin givet ved udgangsmodstanden for det første forstærkertrin, som er lig med $R_{C1}$ hvilket gør at $R'_S = R_{C1} || R_3 || R_4$. Den maksimale $R_{C2}$ beregnes i ligning \ref{eq:rc2maks}.
...
Spændingsforstærkningen af hele trinnet skal være 63,3 gange svarende til 36 dB. Forstærkningen vises på figur \ref{fig:amplitude-forforstaerker} ved hjælp af en amplitudekarakteristik gående fra 0 Hz til 40 kHz, således at spændet fra 20 Hz til 20 kHz tydeligt kan ses. 
...
Simuleringen viser at forstærkningen ved 1 kHz, som er referencen jævnfør kravspecifikationen, er 35,2 dB hvor den skulle have være 36 dB. Dermed må der konkluderes at beregningen af forforstærkertrinnene indeholder usikkerheder. Usikkerhederne består i de antagelser som defineres før beregningerne: $r_o >>R_C \| R_L$, $gm \cdot R_E||R_e$ og $i_e \approx i_c$. For at korrigere forstærkningen i trinnene kan der, ifølge ligning \ref{eq:gmbevis}, justeres på den AC-koblede emittermodstand, $R_{e1}$ og $R_{e2}$. I ligning \ref{eq:nyerevalues} er de nye værdier for disse modstanden anvist.

\begin{equation}
2
\end{equation}

Simuleringen viser tydeligt frekvensgangen er, som den skal være fra 20 Hz til 20 kHz. Ved 20 Hz er der en dæmpning på 2 dB 

\begin{figure}[h]
\centering
\includegraphics[scale=.6]{teknisk/forforstaerker/svingningforforstaerker.png}
\caption{Forforstærkerens output ved }
\label{fig:forforstaerkersvingning}
\end{figure}
